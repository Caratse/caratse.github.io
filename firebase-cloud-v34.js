(() => {
  ...
  window.CloudBudgetReady = (async () => {
    ...
    const db = initializeFirestore(app, {
      ...
      // Compatibility mode for Safari/PWA networks, proxies and VPN routes
      // that fail to keep Firestore's normal WebChannel connection open.
    ...
    function friendlyCloudError(error, action = "Cloud request") {
      ...
      if (
        ...
        /failed to get documents from server/i.test(message) ||
        /client is offline/i.test(message) ||
        /network/i.test(message)
      ) {
        const wrapped = new Error(
          `${action} could not reach the Firebase server. ` +
          `Try another network or reconnect the VPN, reopen Daily Budget, ` +
          `then tap Verify cloud data.`
        );
        wrapped.code = code || "server-unreachable";
        return wrapped;
      }

      if (code === "permission-denied") {
        const wrapped = new Error(
          `${action} was rejected by Firestore rules. ` +
          `Do not import again. Check the deployed Firestore rules first.`
        );
        wrapped.code = code;
        return wrapped;
      }
      ...
      if (code === "unauthenticated") {
        const wrapped = new Error(
          `${action} needs a fresh sign-in. Sign out, sign in again, ` +
          `then tap Verify cloud data.`
        );
        wrapped.code = code;
        return wrapped;
      ...
      const wrapped = new Error(
        `${action} failed${code ? ` (${code})` : ""}: ${message || "Unknown error"}`
      );
    ...
    async function fetchServerState(uid) {
      ...
      const [transactionSnapshot, settingsSnapshot, trashSnapshot] = await Promise.all([
        ...
        throw friendlyCloudError(error, "Server verification");
      }
    }
